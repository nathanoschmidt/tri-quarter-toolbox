...
the subproject's eight falsifiable claims (C1-C8), so that the experiments and the
...
|-------|------------------|----------------------|
| **C1** | Closed-form decode is bitwise-identical to exhaustive ML; the 3x3 window equals a wide brute-force nearest-point search (incl. deep holes); folding preserves bitwise-ML | `test_decode_fast_equals_ml_on_noisy_stream`, `test_decode_fast_equals_ml_on_dense_grid`, `test_nearest_lattice_point_matches_bruteforce`, `test_folded_decoder_matches_ml_dense_grid`, `test_folded_decoder_matches_ml_monte_carlo` |
...
3. State the claim (C1-C8) the test supports in the docstring.
